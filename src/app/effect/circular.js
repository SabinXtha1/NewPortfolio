...
const CircularText = ({
  ...
  return (
    <motion.div
      initial={{ rotate: 0 }}
      className={`mx-auto rounded-full w-[100px] h-[100px] text-white font-black text-center cursor-pointer ${className} z-10 text-xl `}
      animate={controls}
    ...
    > 
      {letters.map((letter, i) => {
        ...
        return (
          <span
            key={i}
            className="absolute inline-block inset-0 font-extralight tracking-wide   transition-all duration-500 ease-[cubic-bezier(0,0,0,1)] text-[10px]"
